...
import { domainEventBus, DomainEventPayload } from '@corecart/shared';
  ...
  private registerDomainEventSubscribers() {
    domainEventBus.subscribe("UserRegistered", async (evt) => {
      await this.sendNotification({
        ...
        payload: evt.payload,
        title: `Welcome to ${process.env.NEXT_PUBLIC_APP_NAME || 'the store'}!`,
        body: `Hi ${evt.payload.firstName || "there"}, welcome to ${process.env.NEXT_PUBLIC_APP_NAME || 'the store'}!`,
